...
- **Primera versión de UNPOSER v2.** Reescritura completa abandonando Reflex en favor de un stack desacoplado: **React + Vite + TypeScript + Tailwind + shadcn/ui** en el frontend y **FastAPI** en el backend, empaquetados en una **única imagen Docker**.
...
- Workflow `despliegue.yml`: en cada push a `main` o `develop` crea tag + release en GitHub y publica imagen multiarch (`amd64`, `arm64/v8`, `arm/v7`) en **DockerHub** (`unraiders/unposerv2`) y **GHCR** (`ghcr.io/unraiders/unposerv2`).
